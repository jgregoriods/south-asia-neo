...
GA <- function(numGenes, numGenomes, numParents, numElite, mutationRate, numIter, cores=NULL) {
    ...
    clusterExport(cl, varlist=c("BIOMES", "ORIGIN", "START", "DATES", "reclassRaster",
                                "testModel", "simulateDispersal", "elev.test", "slope.test",
				"robust.factor", "alpha.fun", "gamma.fun", "huber.M", "line.cis", "sma"),
                                envir=environment())
...
start_time <- Sys.time()
res <- GA(numGenes, numGenomes, numParents, numElite, mutationRate, numIter, 18)
total_time <- Sys.time() - start_time
cat("Completed in",total_time[[1]],attributes(total_time)$units,"\n")
save(res, file="ga2610marvin.RData")
